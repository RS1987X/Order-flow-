# -*- coding: utf-8 -*-
"""
Created on Mon Feb 21 20:40:24 2022
...
dates = (["20220128","20220131","20220201","20220202","20220203","20220204","20220207",
         ...
         "20220217","20220218","20220221"])
#dates = ["20220215","20220216","20220217","20220218","20220221"]
#dates = ["20220214","20220215","20220216","20220217","20220218"]
#dates = ["20220211","20220214","20220215","20220216","20220217"]
...
for x in tickers:
    for d in dates:
        ...
        if d == "20220128":
            ...
            opening_auction_price = float(data.iloc[:,2][data.iloc[:,8].str.contains(pat = "OA")].iloc[-1].replace(",","."))
            opening_auction_volume = data.iloc[:,4][data.iloc[:,8].str.contains(pat = "OA")]
            opening_seller = opening_auction_price < yest_close
            opening_buyer = opening_auction_price > yest_close
        
        tot_buy_volume = buyer*data.iloc[:,4] + opening_buyer*opening_auction_volume
        tot_sell_volume = seller*data.iloc[:,4] + opening_seller*opening_auction_volume
...
# =============================================================================
        ...
        close_price = data.loc[(data.iloc[:,1] > "17:25:00") & (data.iloc[:,1] < "17:30:00"),2]
        ...
        #mid-day-return
        mid_day_price = data.loc[(data.iloc[:,1] > "10:50:00") & (data.iloc[:,1] < "11:00:00"),2]
        #close_price = float(data.iloc[0,2].replace(",","."))
        mid_day_price = mid_day_price.apply(lambda x: x.replace(',','.')).astype(float)
        mid_day_price = mid_day_price.mean()
...
# =============================================================================
        ...
        ret_close = close_price/entry_price-1
        ret_close_str = "{:.1%}".format(ret_close)
        
        mid_day_ret = mid_day_price/entry_price-1
        
        print("Post open return to close " + ret_close_str)
        print(" ")
        yest_close = close_price
        of_data.append((x,percent_of_opening_vol,ret_close,mid_day_ret))
        #of_data.append((x,d,opening_of,ret_close))



#percent of volume on bid/ask
percent_of_opening_vols = [x[1] for x in of_data]
returns_percent = [x[2] for x in of_data]

#percent_vol_data.sort(key = lambda x: x[1])
percent_vol_data_df = pd.DataFrame(of_data, columns=['name', 'percent_of_opening_vol', 'close returns','mid day returns'])
threshold = 0.4
...
strat_ret = longs*stocks_w_signal["close returns"] - shorts*stocks_w_signal["close returns"]
...
plt.figure(2)
plt.scatter(stocks_w_signal["percent_of_opening_vol"],stocks_w_signal["close returns"])
lm2 = sm.OLS(stocks_w_signal["close returns"],stocks_w_signal["percent_of_opening_vol"],missing='drop').fit()
print(lm2.summary())

#afternoon session return/morning session return correlation
afternoon_return = (stocks_w_signal["close returns"]+1)/(stocks_w_signal["mid day returns"]+1)-1
plt.figure(3)
plt.scatter(stocks_w_signal["mid day returns"],afternoon_return)
lm2 = sm.OLS(stocks_w_signal["mid day returns"],afternoon_return,missing='drop').fit()
print(lm2.summary())

pm_ret_neg_am = afternoon_return[stocks_w_signal["mid day returns"]<0].mean()
pm_ret_pos_am = afternoon_return[stocks_w_signal["mid day returns"]>0].mean()
