# -*- coding: utf-8 -*-
"""
Created on Thu Feb 17 21:31:35 2022
...
# -*- coding: utf-8 -*-
"""
Created on Wed Feb 16 23:19:13 2022
...
tickers = (["EVO","SINCH","LATO_B","KINV_B","NIBE_B","EQT","MIPS","STORY_B",
...
for d in dates:
    ...
    for x in tickers:
        ...
        buyer = (data.iloc[:,11] == "<") & (data.iloc[:,7] == "HID") 
        seller = (data.iloc[:,11] == ">") & (data.iloc[:,7] == "HID")
        ...
        not_opening_closing_call = data.iloc[:,11].notna()
        opening_period = data.iloc[:,1] < "10:00:00"
...
# =============================================================================
        #entry price
        entry_period_volumes = data.loc[(data.iloc[:,1] > "10:05:00") & (data.iloc[:,1] < "10:10:00"),4]
        entry_period_prices = data.loc[(data.iloc[:,1] > "10:05:00") & (data.iloc[:,1] < "10:10:00"),2]
...
plt.figure(3)
plt.scatter(of_df[of_df!=0],returns_df[of_df!=0])
lm = sm.OLS(returns_df[of_df!=0].stack(),of_df[(of_df!=0) & ~returns_df.isna()].stack(),missing='drop').fit()
print(lm.summary())
...
# lm = sm.OLS(returns_of_pos.stack(),of_pos_n_std_dev.stack(),missing='drop').fit()
# print(lm.summary())
# 
# plt.figure(4)
# plt.scatter(of_neg_n_std_dev,returns_of_neg)
# lm = sm.OLS(returns_of_neg.stack(),of_neg_n_std_dev.stack(),missing='drop').fit()
# print(lm.summary())
# 
# plt.figure(5)
# plt.scatter(of_neg_n_std_dev[(of_neg_n_std_dev<-1.5) | (of_neg_n_std_dev>1.5)].stack(),returns_of_neg[(of_neg_n_std_dev<-1.5) | (of_neg_n_std_dev>1.5)].stack())
# lm = sm.OLS(returns_of_neg[(of_neg_n_std_dev<-1.5) | (of_neg_n_std_dev>1.5)].stack(),of_neg_n_std_dev[(of_neg_n_std_dev<-1.5) | (of_neg_n_std_dev>1.5)].stack(),missing='drop').fit()
# print(lm.summary())
# 
# =============================================================================
